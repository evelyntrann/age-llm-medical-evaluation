...
The objective is to build a clinically grounded counterfactual benchmark that measures appropriate adaptation and two complementary errors: over-conditioning when age should not matter, and under-conditioning when age should matter.
...
**Benchmark.** Curate a candidate pool of up to 1,000 first-person medical scenarios from sources with verified licenses, with a smaller clinician-validated core prioritized over raw scale. Remove nonmedical items, caregiver/third-person questions, explicit-age cases, duplicates, protected health information, and cases whose facts cannot be held constant. Clinicians will classify core scenarios as age-invariant, age-relevant, or age-conditional, stratify them by specialty and acuity, and specify which clinical response dimensions should remain invariant or adapt. Each retained scenario will receive a neutral condition and deterministic exact-age variants (for example, 16, 30, 50, and 75 years). Only the age sentence will change; an LLM will not rewrite symptoms, duration, or history. A small secondary experiment may compare numeric ages with labels such as “older adult.”
